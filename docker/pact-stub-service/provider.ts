...
export const getProvider = ({ pactPort, provider }: PactProviderOptions) => {
  return new Pact({
    consumer: "signing-service", // current service at hand, it makes it easier to know who would be broken by the change in the provider when we test the contract.
